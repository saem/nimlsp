...
type
  NilType* = enum Nil
  DefKind = tuple[name: string, isArray: bool]
  DefKinds = seq[DefKind]
...
proc extractKinds(node: NimNode): seq[tuple[name: string, isArray: bool]] =
  if node.kind == nnkIdent:
    ...
    raise newException(AssertionDefect, "Array of nils not allowed")
  else:
    raise newException(AssertionDefect, "Unknown node kind: " & $node.kind)
...
macro jsonSchema*(pattern: untyped): untyped =
  ...
  var
    typeDefinitions = initOrderedTable[string, NimNode]()
  ...
  for t in types:
    ...
    typeDefinitions[t.name] = quote do:
      type
    ...
    for field in t.definitions:
      ...
      for kind in field.kinds:
        let
          kindName = if kind.name == "any": "JsonNode" else: kind.name
          tKind = if kind.isArray:
  ...
  var typeDefns = newStmtList()
  for d in typeDefinitions.values: typeDefns.add(d)
...
when isMainModule:
  ...
  var
    wcp = create(WrapsCancelParams,
      create(CancelParams, some(10), none(float)),
      none(CancelParams),
      "Hello"
    )
    wcp2 = create(WrapsCancelParams,
      create(CancelParams, some(13), none(float)),
      some(create(CancelParams, none(int), none(float))),
      "I'm different"
    )
  ...
  echo wcp2.JsonNode.isValid(WrapsCancelParams) == true
